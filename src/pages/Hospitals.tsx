...
export default function Hospitals() {
  ...
  const cities = useMemo(
    () => Array.from(new Set(data.hospitals.map((h) => h.city))),
    [data.hospitals],
  );
  ...
  }, [data.hospitals, q, city, onlyEmergency]);
    ...
    <PageWrap>
      <Reveal>
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground">
          Partner hospitals across India with active blood banks. Sample directory for the demo.
        </p>
      </Reveal>
      ...
      <p className="mt-5 text-xs font-semibold text-muted-foreground">
        {results.length} of {data.hospitals.length} hospitals
      </p>
      ...
      {results.length === 0 ? (
        <div className="mt-4">
          <EmptyState
            ...
            message="Try a different keyword or clear the emergency filter."
          />
        </div>
      ) : (
            ...
            <Reveal key={h.id} delay={(i % 3) * 0.06}>
              <div className="clay-card flex h-full flex-col p-6 transition-transform duration-200 hover:-translate-y-1.5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {h.emergency24x7 && (
                    <Pill tone="redsolid"><Siren className="size-3.5" /> 24×7 Emergency</Pill>
                  )}
                </div>
                <h3 className="mt-4 text-base font-extrabold leading-snug text-foreground">{h.name}</h3>
                <p className="mt-1.5 flex items-start gap-1.5 text-xs text-muted-foreground">
                  <MapPin className="mt-0.5 size-3.5 shrink-0" /> {h.address}, {h.city}
                </p>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {h.groups.map((g) => (
                      <span key={g} className="rounded-lg bg-primary/10 px-2 py-1 text-xs font-extrabold text-primary">{g}</span>
                    ))}
                  </div>
                </div>
                <div className="mt-auto flex gap-2 pt-5">
                  <Button size="sm" className={cn(btnPrimary, "flex-1")} onClick={() => setSelected(h)}>
                    View details
                  </Button>
                  <Button asChild size="sm" variant="secondary" className={cn(btnSoft, "flex-1")}>
                    <Link to={`/find-blood?city=${h.city}`}>Find blood</Link>
                  </Button>
                </div>
              </div>
            </Reveal>
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="text-left">
                  Partner hospital · {selected.city}
                </DialogDescription>
              </DialogHeader>
              <div className="clay-tile grid gap-2.5 p-4 text-sm">
                ...
                <p className="flex items-center gap-2">
                  <Siren className="size-4 text-primary" /> Emergency:{" "}
                  {selected.emergency24x7 ? <Pill tone="redsolid">Open 24×7</Pill> : <Pill tone="amber">Day hours only</Pill>}
                </p>
              ...
              <Button asChild className={cn(btnPrimary, "mt-4 w-full")}>
                <Link to={`/requests?blood=${selected.groups[0]}&city=${selected.city}`}>Request blood here</Link>
              </Button>
            </>
          )}
        </DialogContent>
