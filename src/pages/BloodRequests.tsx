...
const statusFilters = ["all", "pending", "approved", "fulfilled", "rejected"] as const;
...
export default function BloodRequests() {
  ...
  const submit = (e: FormEvent) => {
    ...
    else if (new Date(form.neededBy) < new Date(new Date().toDateString()))
      errs.neededBy = "Required date cannot be in the past.";
    setErrors(errs);
    ...
    toast.success("Blood request submitted!", {
      description: "Hospitals and donors near you can now see it on the board (demo).",
    });
    setForm((f) => ({ ...f, patientName: "", notes: "", hospitalName: "" }));
  };

  const visible: BloodRequest[] =
    ...
    <PageWrap>
      <Reveal>
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground">
          Raise a request for a patient and track every request on the live board.
        </p>
      </Reveal>
        ...
        <Reveal delay={0.05}>
          <form className="clay-card grid gap-4 p-6 sm:grid-cols-2 sm:p-8" onSubmit={submit} noValidate>
            ...
            <Button size="lg" className={cn(btnPrimary, "sm:col-span-2")}>
              <Droplet className="size-5" /> Submit Request
            </Button>
          </form>
        </Reveal>
                ...
                <li key={r.id} className="clay-tile p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {canManage && r.status === "pending" && (
                      <>
                        <Button size="sm" className={btnPrimary} onClick={() => { setRequestStatus(r.id, "approved"); toast.success("Request approved."); }}>
                          <CheckCircle2 className="size-4" /> Approve
                        </Button>
                        <Button size="sm" variant="secondary" className={btnSoft} onClick={() => { setRequestStatus(r.id, "rejected"); toast.info("Request rejected."); }}>
                          <XCircle className="size-4" /> Reject
                        </Button>
                      </>
                    )}
                    {canManage && (r.status === "approved" || r.status === "pending") && (
                      <Button size="sm" className={cn(btnPrimary, "bg-emerald-600 hover:bg-emerald-600/90")} onClick={() => { setRequestStatus(r.id, "fulfilled"); toast.success("Request fulfilled — inventory updated."); }}>
                        <Droplet className="size-4" /> Mark fulfilled
                      </Button>
                    )}
                    {(canManage || r.requestedBy === user?.id) && (
                      <Button size="sm" variant="ghost" className="text-destructive hover:bg-red-50" onClick={() => setConfirmDelete(r.id)}>
