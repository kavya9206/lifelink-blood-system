...
import { isApiMode, apiCreateRequest, apiStats } from "@/lib/api";
import { requestPayloadForApi } from "@/lib/mappers";
import { BLOOD_GROUPS, CITIES } from "@/lib/types";
import { Field, PageWrap, Reveal, SectionHeading, StatCard, btnPrimary, btnSoft, inputCls, selectCls, } from "@/components/clay";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { ArrowRight, Baby, BedDouble, Building2, CalendarCheck2, ClipboardCheck, Droplet, Droplets, HeartHandshake, MapPin, PhoneCall, Search, ShieldCheck, Stethoscope, Users, Zap, } from "lucide-react";
...
const groupDot = (units) => units <= 5 ? "bg-red-500" : units <= 12 ? "bg-amber-500" : "bg-emerald-500";
export default function Landing() {
    ...
    const submitEmergency = async (e) => {
        e.preventDefault();
        const errs = {};
        if (em.patientName.trim().length < 3)
            errs.patientName = "Please enter the patient's full name.";
        if (!/^[+]?[\d\s-]{10,15}$/.test(em.contact.trim()))
            errs.contact = "Enter a valid phone number (10+ digits).";
        if (em.hospitalName.trim().length < 3)
        ...
        if (isApiMode) {
            ...
            try {
                ...
                toast.success("Emergency request submitted to Supabase!", { description: `${em.units} unit(s) of ${em.bloodGroup} — hospitals have been alerted.` });
                setEm({ patientName: "", bloodGroup: "O+", units: 1, hospitalName: "", hospitalCity: "Mumbai", contact: "" });
            } catch (err) { toast.error(err instanceof Error ? err.message : String(err)); } finally { setEmBusy(false); }
            return;
        }
        addRequest(payload, null);
        toast.success("Emergency request submitted!", { description: `${em.units} unit(s) of ${em.bloodGroup} — nearby blood banks have been alerted (demo).`, });
        setEm({ patientName: "", bloodGroup: "O+", units: 1, hospitalName: "", hospitalCity: "Mumbai", contact: "" });
    };
    return (<PageWrap className="overflow-x-clip">
      {/* ---------------- HERO ---------------- */}
      <section className="relative">
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.15, ease: [0.22, 1, 0.36, 1] }} className="relative">
            <div className="clay-card relative mx-auto max-w-md p-7">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {BLOOD_GROUPS.map((g) => (<div key={g} className="clay-tile flex items-center justify-between px-4 py-3">
      ...
      <section className="mt-24">
        <SectionHeading eyebrow="Why Donate" title={<>A small act with a <span className="text-gradient-red">huge ripple</span></>} subtitle="One unit of donated blood can be separated into red cells, plasma and platelets — helping several different patients."/>
        ...
        ].map((f, i) => (<Reveal key={f.title} delay={i * 0.08}>
      ...
      {/* ---------------- HOW IT WORKS ---------------- */}
      <section className="mt-24">
        <SectionHeading eyebrow="How It Works" title="From sign-up to life-saved in four steps"/>
        ...
        ].map((s, i) => (<Reveal key={s.n} delay={i * 0.08}>
            ...
            <div className="grid lg:grid-cols-[1fr_1.2fr]">
              ...
              <form className="grid gap-4 p-8 sm:grid-cols-2 sm:p-10" onSubmit={submitEmergency} noValidate>
                <Field label="Patient name" error={emErrors.patientName} className="sm:col-span-2">
                  <input className={inputCls} placeholder="e.g. Ramesh Gupta" value={em.patientName} onChange={(e) => setEm({ ...em, patientName: e.target.value })}/>
                </Field>
                <Field label="Blood group">
                  <select className={selectCls} value={em.bloodGroup} onChange={(e) => setEm({ ...em, bloodGroup: e.target.value })}>
                ...
                <Button size="lg" className={cn(btnPrimary, "sm:col-span-2")} disabled={emBusy}>
                  <Zap className="size-5"/> {emBusy ? "Submitting…" : "Submit emergency request"}
                </Button>
              </form>
            </div>
          </div>
      ...
      {/* ---------------- TESTIMONIALS ---------------- */}
      <section className="mt-24">
        <SectionHeading eyebrow="Testimonials" title="Stories from the LifeLink family" subtitle="Sample testimonials from our demo dataset."/>
        ...
        ].map((t, i) => (<Reveal key={t.name} delay={i * 0.08}>
              <figure className="clay-card flex h-full flex-col p-7">
                <div className="mb-4 flex gap-1" aria-label="5 star rating">
                  {Array.from({ length: 5 }).map((_, s) => (<svg key={s} viewBox="0 0 20 20" className="size-4 fill-amber-400"><path d="M10 1.5 12.6 7l6 .6-4.5 4 1.3 5.9L10 14.4l-5.4 3.1L5.9 11.6l-4.5-4 6-.6Z"/></svg>))}
                </div>
      ...
      <section className="mt-24">
        <Reveal>
          <div className="clay-card relative overflow-hidden p-10 text-center sm:p-14">
            ...
            <button onClick={() => navigate("/blood-bank")} className="mt-6 text-sm font-bold text-primary underline-offset-4 hover:underline">
