...
export default function DonateBlood() {
  ...
  const [form, setForm] = useState<FormState>(
    linkedDonor
      ? {
          name: linkedDonor.name,
          age: linkedDonor.age ? String(linkedDonor.age) : "",
          gender: linkedDonor.gender,
          bloodGroup: linkedDonor.bloodGroup,
          phone: linkedDonor.phone,
          email: linkedDonor.email,
          city: linkedDonor.city,
          address: linkedDonor.address,
          lastDonation: linkedDonor.lastDonation ?? "",
          healthConfirm: true,
          available: linkedDonor.available,
        }
      : initial,
  );
  ...
  const validate = (): boolean => {
    ...
    if (!form.age || Number.isNaN(age) || age < 18 || age > 65)
      e.age = "Donors must be between 18 and 65 years.";
  ...
  const submit = (ev: FormEvent) => {
    ...
    if (linkedDonor) {
      ...
      const donor = addDonor(payload);
      setSuccessId(donor.id);
      toast.success("You're registered as a donor!", {
        description: "Hospitals near you can now see your availability (demo data).",
      });
    }
    ...
    return (
      <PageWrap className="max-w-3xl">
        <div className="clay-card p-10 text-center sm:p-14">
          <span className="mx-auto mb-5 grid size-20 place-items-center rounded-[1.75rem] bg-emerald-100 text-emerald-600 shadow-[inset_0_3px_4px_rgba(255,255,255,.9),inset_0_-8px_14px_rgba(16,120,80,.15)]">
            <BadgeCheck className="size-10" />
          </span>
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground">
            {linkedDonor ? "Profile updated!" : "Welcome aboard, hero!"}
          </h1>
          <p className="mx-auto mt-3 max-w-md text-muted-foreground">
            {saved?.name}, you are registered as an available <b className="text-primary">{saved?.bloodGroup}</b> donor in{" "}
            <b className="text-foreground">{saved?.city}</b>. Hospitals and patients on LifeLink can now find you.
          </p>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Button asChild className={btnPrimary}>
              <Link to={user ? "/dashboard" : "/auth?mode=register"}>
                <UserRound className="size-4" /> {user ? "Open my dashboard" : "Create an account"}
              </Link>
            </Button>
            <Button
              variant="secondary"
            ...
            >
              <RotateCcw className="size-4" /> Register another donor
            </Button>
          </div>
        </div>
      </PageWrap>
    ...
    <PageWrap className="max-w-4xl">
      <Reveal>
        <h1 className="text-4xl font-extrabold tracking-tight text-foreground">
          {linkedDonor ? "Update Donor Profile" : "Donate Blood"}
        </h1>
        <p className="mt-2 max-w-2xl text-muted-foreground">
          {linkedDonor
            ? "Keep your details current so hospitals can reach you when it matters."
            : "Join thousands of voluntary donors. Fill in your details — it takes about two minutes."}
        </p>
      </Reveal>
      ...
      <Reveal delay={0.05} className="mt-8">
        <form className="clay-card grid gap-5 p-6 sm:grid-cols-2 sm:p-10" onSubmit={submit} noValidate>
          <Field label="Full Name" error={errors.name}>
            <input className={inputCls} placeholder="e.g. Aarav Sharma" value={form.name} onChange={(e) => set("name", e.target.value)} />
          </Field>
          <Field label="Age" error={errors.age} hint="18–65 years">
            <input className={inputCls} type="number" min={18} max={65} placeholder="e.g. 27" value={form.age} onChange={(e) => set("age", e.target.value)} />
          </Field>
          ...
          <Field label="Phone Number" error={errors.phone}>
            <input className={inputCls} inputMode="tel" placeholder="+91 98XXX XXXXX" value={form.phone} onChange={(e) => set("phone", e.target.value)} />
          </Field>
          <Field label="Email" error={errors.email}>
            <input className={inputCls} type="email" placeholder="you@example.in" value={form.email} onChange={(e) => set("email", e.target.value)} />
          </Field>
          ...
          <Field label="Last Donation Date" error={errors.lastDonation} hint="Leave blank if this is your first donation">
            <input className={inputCls} type="date" max={new Date().toISOString().slice(0, 10)} value={form.lastDonation} onChange={(e) => set("lastDonation", e.target.value)} />
          </Field>
          <Field label="Address" error={errors.address} className="sm:col-span-2">
            <textarea className={cn(inputCls, "h-24 resize-none py-3")} placeholder="House / street / locality" value={form.address} onChange={(e) => set("address", e.target.value)} />
          </Field>
          ...
          <div className="clay-tile flex flex-col gap-3 p-5 sm:col-span-2">
            <label className="flex cursor-pointer items-start gap-3 text-sm">
              <input
                type="checkbox"
                className="mt-0.5 size-4 shrink-0 accent-[#e5484d]"
                checked={form.healthConfirm}
                onChange={(e) => set("healthConfirm", e.target.checked)}
              />
              <span className="leading-relaxed">
                <b>Eligibility confirmation:</b> I am between 18–65 years, weigh over 50 kg, and do
                not have fever, infection, or a recent surgery / tattoo (within 6 months). I confirm
            ...
            <div className="flex items-center justify-between gap-3 border-t border-border pt-3">
              ...
              <div className="flex gap-2">
                {[
                  { v: true, label: "Available", icon: <Droplet className="size-3.5" /> },
                  { v: false, label: "Not available", icon: <CalendarDays className="size-3.5" /> },
                ].map((o) => (
                  <button
                    type="button"
                    key={o.label}
                    onClick={() => set("available", o.v)}
                    className={cn(
          ...
          <div className="flex flex-wrap items-center gap-3 sm:col-span-2">
            <Button size="lg" className={btnPrimary}>
              <HeartHandshake className="size-5" /> {linkedDonor ? "Save changes" : "Register as donor"}
            </Button>
            <p className="text-xs text-muted-foreground">
              Demo app — data is stored only in your browser's localStorage.
            </p>
          </div>
        </form>
      </Reveal>
