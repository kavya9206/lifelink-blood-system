...
import { inventoryStatus, BLOOD_GROUPS, CITIES, type BloodGroup } from "@/lib/types";
...
const groupStatus = (units: number): "available" | "low" | "critical" =>
  units <= 8 ? "critical" : units <= 18 ? "low" : "available";
...
export default function BloodBank() {
  ...
  const groups = useMemo(() => {
    return BLOOD_GROUPS.map((g) => {
      const items = data.inventory.filter((i) => i.bloodGroup === g);
      const units = items.reduce((n, i) => n + i.units, 0);
      const last = items.reduce((m, i) => (i.updatedAt > m ? i.updatedAt : m), "");
      return { group: g as BloodGroup, units, status: groupStatus(units), lastUpdated: last };
    });
  }, [data.inventory]);
      ...
      <Reveal className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-muted-foreground">
            Live demo stock across {new Set(data.inventory.map((i) => i.city)).size} blood banks —{" "}
            <b className="text-foreground">{totals.units} units</b> in total. Inventory updates
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-5">
        {groups.map((g, i) => (
          <Reveal key={g.group} delay={i * 0.05}>
            <div
              className={cn(
                "clay-card p-5 transition-transform duration-200 hover:-translate-y-1",
                g.status === "critical" && "ring-2 ring-red-300/70",
              )}
            >
              ...
              <div className="mt-4 h-2.5 w-full overflow-hidden rounded-full bg-muted shadow-[inset_0_2px_3px_rgba(146,47,47,.12)]">
                <div
                  className={cn("h-full rounded-full transition-all duration-700", barTone[g.status])}
                  style={{ width: `${Math.min(100, (g.units / CAPACITY) * 100)}%` }}
                />
              </div>
              ...
              <div className="mt-3 flex items-center justify-between text-xs">
                {g.status !== "available" ? (
                  <span className="flex items-center gap-1 font-bold text-red-600">
                    <AlertTriangle className="size-3.5" /> {g.status === "critical" ? "Critical — donations needed!" : "Low stock"}
                  </span>
                ) : (
                  <span className="flex items-center gap-1 font-semibold text-emerald-600">
                    <Droplets className="size-3.5" /> Healthy stock
                  </span>
                )}
      ...
      <Reveal delay={0.1} className="mt-12">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <h2 className="flex items-center gap-2 text-lg font-extrabold text-foreground">
            <Warehouse className="size-5 text-primary" /> Facility-wise stock
          </h2>
          <div className="ml-auto flex flex-wrap gap-2">
            <button
              ...
              className={cn(
                ...
                city === "all" ? "bg-primary text-primary-foreground" : "clay-inset text-muted-foreground hover:text-foreground",
              )}
            >
              All cities
            </button>
            {CITIES.filter((c) => data.inventory.some((i) => i.city === c)).map((c) => (
              <button
                key={c}
                onClick={() => setCity(c)}
                className={cn(
                  "rounded-full px-3.5 py-1.5 text-xs font-bold transition-all",
                  city === c ? "bg-primary text-primary-foreground" : "clay-inset text-muted-foreground hover:text-foreground",
                )}
              >
                {c}
              </button>
        ...
        <p className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
          ...
          Sample inventory for illustration — real availability must always be confirmed with the blood bank.
        </p>
      </Reveal>
    </PageWrap>
  );
