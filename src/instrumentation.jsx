...
function ErrorDialog({ error, setError, }) {
    const technicalDetails = [
        ...
        .filter(Boolean)
        .join("\n\n");
    return (<Dialog defaultOpen={true} onOpenChange={(open) => {
            if (!open)
                setError(null);
        }}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto border-zinc-700 bg-zinc-950 text-zinc-100 sm:max-w-lg">
        ...
        {technicalDetails && (<Collapsible>
            <CollapsibleTrigger asChild>
              <button type="button" className="flex cursor-pointer items-center gap-1.5 text-xs font-medium text-zinc-400 transition-colors hover:text-zinc-100">
...
class ErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError && this.state.error) {
            return (<ErrorDialog error={this.state.error} setError={(error) => {
                    if (error === null) {
