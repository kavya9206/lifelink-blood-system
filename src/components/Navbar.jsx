...
export function Navbar() {
    ...
    const dashboardLabel = user?.role === "admin" || user?.role === "hospital" ? "Admin Dashboard" : "My Dashboard";
    return (<header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-6">
      <div className="clay-card mx-auto flex h-16 w-full max-w-7xl items-center justify-between rounded-full px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((l) => (<NavLink key={l.to} to={l.to} end={l.to === "/"} className={({ isActive }) => cn("rounded-2xl px-3.5 py-2 text-sm font-semibold transition-colors", isActive
        ...
        <div className="flex items-center gap-2">
          {user ? (<DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="clay-card w-56 border-0 p-2">
                ...
                {(user.role === "admin" || user.role === "hospital") && (<DropdownMenuItem className="rounded-xl" onClick={() => navigate("/admin")}>
                    <ShieldCheck /> {dashboardLabel}
                  </DropdownMenuItem>)}
                {user.role !== "admin" && user.role !== "hospital" && (<DropdownMenuItem className="rounded-xl" onClick={() => navigate("/dashboard")}>
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem className="rounded-xl text-destructive" onClick={() => {
                if (isApiMode)
                    apiLogout();
                logout();
                navigate("/");
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="clay-card w-72 border-0 p-5">
              ...
              <nav className="mt-6 flex flex-col gap-1">
                {NAV_LINKS.map((l) => (<NavLink key={l.to} to={l.to} end={l.to === "/"} onClick={() => setOpen(false)} className={({ isActive }) => cn("rounded-2xl px-4 py-2.5 text-sm font-semibold", isActive ? "bg-secondary text-primary" : "text-muted-foreground hover:bg-secondary/60")}>
                    {l.label}
                  </NavLink>))}
              </nav>
              {!user && (<div className="mt-6 grid gap-2">
              ...
              {user && (<Button variant="secondary" className={cn(btnSoft, "mt-6 font-bold text-destructive")} onClick={() => {
                setOpen(false);
                if (isApiMode)
                    apiLogout();
                logout();
