...
export function DropMark({ className }) {
    return (<svg viewBox="0 0 24 24" className={cn("size-7", className)} aria-hidden>
      <path d="M12 2.4c3.2 4.2 6.6 8 6.6 12.1A6.6 6.6 0 0 1 12 21.6a6.6 6.6 0 0 1-6.6-7.1C5.4 10.4 8.8 6.6 12 2.4Z" fill="currentColor"/>
      <ellipse cx="9.6" cy="13.4" rx="1.7" ry="2.3" fill="rgba(255,255,255,.55)" transform="rotate(-18 9.6 13.4)"/>
    </svg>);
}
export function LifeLinkLogo({ className }) {
...
export function SectionHeading({ eyebrow, title, subtitle, align = "center", }) {
    return (<Reveal className={cn("mb-10 max-w-2xl", align === "center" && "mx-auto text-center")}>
      {eyebrow && (<span className="mb-3 inline-block rounded-full bg-secondary px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-secondary-foreground">
          {eyebrow}
        </span>)}
...
export function Reveal({ children, className, delay = 0, }) {
    return (<motion.div initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.55, delay, ease: [0.22, 1, 0.36, 1] }} className={className}>
      {children}
    </motion.div>);
}
/* ---------------- Data display ---------------- */
...
export function Pill({ tone = "gray", children, className, }) {
    return (<span className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold", badgeTones[tone], className)}>
      {children}
    </span>);
}
...
export function StatusPill({ status }) {
    const s = STATUS_PILL[status] ?? { tone: "gray", label: status };
    return <Pill tone={s.tone}>{s.label}</Pill>;
}
export function StatCard({ icon, value, label, sub, className, }) {
    return (<div className={cn("clay-tile group p-5 transition-transform duration-200 hover:-translate-y-1", className)}>
...
export function EmptyState({ icon, title, message, action, }) {
    return (<div className="clay-tile flex flex-col items-center px-6 py-14 text-center">
    ...
    return (<AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="clay-card border-0">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction className="clay-btn rounded-2xl bg-destructive text-white hover:bg-destructive/90" onClick={onConfirm}>
            {confirmLabel}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
export function Field({ label, htmlFor, error, hint, children, className, }) {
    return (<div className={cn("flex flex-col gap-1.5", className)}>
      ...
      {children}
      {error ? (<p className="text-xs font-semibold text-destructive">{error}</p>) : hint ? (<p className="text-xs text-muted-foreground">{hint}</p>) : null}
    </div>);
}
export const inputCls = "clay-inset h-11 w-full rounded-2xl px-4 text-sm text-foreground placeholder:text-muted-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40";
...
export const btnSoft = "clay-btn-soft rounded-2xl border-0 bg-card text-foreground hover:bg-secondary";
